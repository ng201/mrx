...
opts=MRXLoad();
[x,alpha,exitflag]=MRXSLscs(opts,[1 zeros(1,cols-1)],A,b,Aeq,beq,[zeros(cols_b,1); -inf*ones(K*M,1); zeros(M*Q,1)],[],[]);
if exitflag~=1
    error('Linear program solver exited with error code %d.',exitflag);
end
disp(' ');
